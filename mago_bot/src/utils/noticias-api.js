  ...
  try {
    ...
    if (response.data.status === 'ok') {
      if (response.data.articles && response.data.articles.length > 0) {
        ...
        const noticiasTraduzidas = await Promise.all(response.data.articles.slice(0, 5).map(async (article) => {
          const tituloTraduzido = await traduzirTexto(article.title);
          const descricaoTraduzida = await traduzirTexto(article.description || 'Sem descrição disponível');
