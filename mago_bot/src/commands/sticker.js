...
async function createStaticSticker(inputPath, outputPath) {
  try {
    ...
    while (stickerSize > MAX_STATIC_SIZE && attempts < 5) {
      console.log('A figurinha estática está muito grande, compactando mais...');
      attempts++;
      await sharp(inputPath)
      ...
      console.log(`Tamanho após compressão agressiva (tentativa ${attempts}): ${stickerSize} bytes`);
    }
  ...
  try {
    ...
    if (message && message.videoMessage) {
      ...
      if (videoMessage.url) {
        ...
        mediaFile = await downloadMediaMessage(msg, { logger: console, decrypt: true });

        if (!mediaFile) {
          console.error('Falha ao baixar o vídeo.');
          await sock.sendMessage(msg.key.remoteJid, { text: 'Erro ao baixar o vídeo. Tente novamente.' });
          return;
        }
    ...
    // Verifica se a mensagem contém uma imagem
      ...
      if (imageMessage.url) {
        ...
        if (!mediaFile) {
          ...
          await sock.sendMessage(msg.key.remoteJid, { text: 'Erro ao baixar a imagem. Tente novamente.' });
          return;
        }
      ...
      await sock.sendMessage(msg.key.remoteJid, { text: 'Por favor, envie uma imagem ou vídeo.' });
      return;
    }
    ...
    if (!fs.existsSync(mediaFilePath)) {
      console.error(`Arquivo de mídia não encontrado no caminho: ${mediaFilePath}`);
      await sock.sendMessage(msg.key.remoteJid, { text: 'Erro ao salvar ou processar a mídia. Tente novamente.' });
      return;
    } else {
    ...
    if (message.videoMessage) {
      ...
      if (!fs.existsSync(stickerPath)) {
        ...
        await sock.sendMessage(msg.key.remoteJid, { text: 'Erro ao criar figurinha animada. Tente novamente.' });
        return;
      }

      const stickerBuffer = fs.readFileSync(stickerPath);
      await sock.sendMessage(msg.key.remoteJid, { sticker: stickerBuffer, gifPlayback: true });
    ...
    // Se for uma imagem, cria uma figurinha estática
      ...
      if (!fs.existsSync(outputStickerPath)) {
        console.error(`A figurinha estática não foi criada: ${outputStickerPath}`);
        await sock.sendMessage(msg.key.remoteJid, { text: 'Erro ao criar figurinha estática. Tente novamente.' });
        return;
      }
    ...
    await sock.sendMessage(msg.key.remoteJid, { text: 'Ocorreu um erro ao processar sua figurinha.' });
  }
}
