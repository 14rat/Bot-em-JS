...
async function checkUrlCommand(msg, sock, args) {
  if (args.length === 0) {
    await sock.sendMessage(msg.key.remoteJid, { text: 'Por favor, forneça uma URL para verificar. Exemplo: !checkurl https://example.com' });
    return;
  }

  const url = args[0];
  const regex = /^(http:\/\/|https:\/\/)([a-zA-Z0-9-]+\.)+[a-zA-Z]{2,7}(\/[a-zA-Z0-9\-_\/]*)?$/;

  if (!regex.test(url)) {
    await sock.sendMessage(msg.key.remoteJid, { text: `❌ URL inválida. Certifique-se de usar um link completo com http:// ou https://.` });
    return;
  }
